...
export class Game {
  ...
  public change_room(direction: string) {
    ...
    else if (direction in this.room_exits) {
  ...
  public display_exits() {
    ...
    const exits_text = Object.keys(room_exits).reduce(
      function(accum, current_exit) {
        return (
          room_exits[current_exit] == null
          ? accum + ""
          : accum + "to the <b>" + current_exit + "</b> lies a " + room_exits[current_exit] + "<br/>"
        );
      },
      ""
    );
  ...
  public examine_room(parsed_input: string[]) {
    ...
    if (valid_element != null) {
      dom_utils.append_text(interactive_object[valid_element] + '<br/><br/>');
    } else {
      dom_utils.append_text('Not much to tell, really. <br/> <br/>');
    }
  }
  ...
  public parse_input(input) {
      let parsedInput = input.toLowerCase().split(" ").filter(element => element !== ">");

      const directionToMove = logic_utils.find_valid_command(parsedInput, DIRECTION_WORDS.concat(this.room_exits));

      console.log(directionToMove)

      if (directionToMove !== null) {
          this.change_room(directionToMove);
      } else if (logic_utils.find_valid_command(parsedInput, INVENTORY_WORDS) !== null) {
          this.display_inventory();
      } else if (parsedInput.includes("help")) {
          dom_utils.help_message();
      } else if (logic_utils.find_valid_command(parsedInput, ["where", "am", "i"]) !== null) {
      ...
      //     this.current_room.examine(parsedInput.slice(1));
      } else if (this.current_room.event != null) {
          this.try_event(parsedInput);
      } else {
