      ...
      event_logic: function(player: Player, flavor_text: string, item_reward: string) {
        dom_utils.append_text(flavor_text);
          ...
          if (!player.has_item(item_reward)) {
              ...
              dom_utils.append_text(`Something catches your eye on the riverbed...<br/><br/> <pre>${art.ring}<pre/> * Amethyst Ring added to inventory *<br/>`);
          }
      }
  },
  pluck: {
      ...
      event_logic: function(player: Player, flavor_text: string, item_reward: string) {
          ...
          if (player.has_item(item_reward)) {
            dom_utils.append_text('Best not to be greedy. <br/><br/>')
          }
          else {
              player.add_item(new Item(item_reward));
